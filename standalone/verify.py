...
import base64, hashlib, itertools, json, math, urllib2, sys, traceback
...
class statusCheck:
	...
	def message(self, message, propagated=False):
		if not propagated:
			if self.onLinePart:
				print()
				self.onLinePart = False
			print(message)
		if self.parent:
			self.parent.message(message, True)
...
with statusCheck("Computing election fingerprint") as sc:
	election_json = json.dumps(election, sort_keys=True)
	fingerprint = base64.b64encode(hashlib.sha256(election_json).digest())[:-1]
	sc.message("Election fingerprint is " + fingerprint)

with statusCheck("Verifying ballots") as sc:
	for i in xrange(0, len(ballots)):
		with statusCheck("Verifying ballot " + str(i), sc) as sc2:
			ballot = ballots[i]
...
with statusCheck("Verifying decryptions") as sc0:
	if trusteeThreshold <= 0:
		for q in xrange(0, numQuestions):
			with statusCheck("Verifying decryptions for question " + str(q), sc0) as sc:
...
if len(verificationProblems) == 0:
	...
	print(results)
	print("Before certifying this election result, please check that the input data corresponds with the data displayed on the election website, including the ballot tracking centre, and remember that this script is not an exhaustive test.")
else:
